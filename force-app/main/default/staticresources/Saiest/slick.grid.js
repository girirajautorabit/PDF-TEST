 ...
 * http://github.com/mleibman/slickgrid1111
 *
 ...
 *     and do proper cleanup.vvcxvcxvcx
 */
	...
	function SlickGrid(container, data, columns, options) {
		...
		function getCanvasNode(columnIdOrIdx, rowIndex) {
			...
			return hasFrozenRows && rowIndex >= actualFrozenRow + (options.frozenBottom ? 0 : 1)
				? hasFrozenColumns() && idx > options.frozenColumn
					? $canvasBottomR[0]
					: $canvasBottomL[0]
				: hasFrozenColumns() && idx > options.frozenColumn
				? $canvasTopR[0]
		...
		function autosizeColumns(autosizeMode, isInit) {
			...
			if (autosizeMode === Slick.GridAutosizeColsMode.FitColsToViewport) {
				if (strColTotalGuideWidth > 0 && totalWidthLessSTR < viewportWidth - strColsMinWidth) {
					...
					(options.viewportSwitchToScrollModeWidthPercent && totalWidthLessSTR + strColsMinWidth > (viewportWidth * options.viewportSwitchToScrollModeWidthPercent) / 100) ||
		...
		function getRowFromNode(rowNode) {
			for (var row in rowsCache) {
				for (var i in rowsCache[row].rowNode) {
					if (rowsCache[row].rowNode[i] === rowNode) return row ? parseInt(row) : 0;
				}
			}
			return null;
		}
